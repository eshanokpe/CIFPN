...
class AdovacyPolicyController extends Controller
{
    public function index(){
        return view('admin.advocacyPolicy.index');
    }

    
    public function policiesGovernanceStore(Request $request)
    {
        try {
            ...
            if ($request->hasFile('image')) {
                ...
                $image->move(public_path('assets/images/policiesGovernance'), $imageName);
            }
    
            PoliciesGovernance::create([
                'title' => $validatedData['title'],
                'content' => $validatedData['content'],
                'image' => 'assets/images/policiesGovernance/'.$imageName
            ]);
    
            return redirect()->back()->with('success', 'Policies Governance added successfully.');
        } catch (ValidationException $e) {
            return redirect()->back()->withErrors($e->validator->errors())->withInput();
        } catch (QueryException $e) {
            return redirect()->back()->with('error', 'Failed to add Policies Governance. Please try again.');
        } catch (\Exception $e) {
            return redirect()->back()->with('error', 'An unexpected error occurred. Please try again later.');
        }
    }
 
    public function policiesGovernanceUpate(Request $request, $id)
    {
        try {
            $validatedData = $request->validate([
                'title' => 'required|string',
            ...
            if ($request->hasFile('image')) {
                ...
                $policiesGovernance->update(['image' =>  'assets/images/policiesGovernance/' . $imageName]);
            } 
            ...
            return redirect()->back()->with('success', 'PoliciesGovernance updated successfully.');
        } catch (ValidationException $e) {
            return redirect()->back()->withErrors($e->validator->errors())->withInput();
